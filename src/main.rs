...
fn run_serve(args: &[String]) {
    ...
    let data_dir = find_flag(args, "--data-dir").unwrap_or_else(|| "data".to_string());
    let outbox_path = std::path::PathBuf::from(&data_dir).join("pending_remote_closes.json");

    let close_outbox = match outbox::RemoteCloseOutbox::load(outbox_path.clone()) {
        Ok(outbox) => outbox,
        Err(e) => {
            eprintln!(
