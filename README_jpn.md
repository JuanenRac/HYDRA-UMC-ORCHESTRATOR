...
> の計画設計です——今日実際に動くもの（最小限のスケルトン）については、
> さらに下の「🔧 ビルドと実行」を参照してください。

現在のスケルトンの上に段階的に構築される**計画中の内部レイヤー**：
* **API 層** — Studio/アプリから高レベルのミッションリクエストを受け取り、フリートレベルのアクションに変換します。
* **ミッションキュー統合** — 受理されたミッションを JOB-DISPATCHER に引き渡し、フリート全体にわたってそのライフサイクルを追跡します。
* **PTP 同期ディスパッチ** — SWARM-SYNC とタイミングを協調させ、同一ミッションを実行する複数のロボットが PATH-PLANNER-3D のチェックに従って衝突のない状態を維持します。
* **フリート健全性の集約** — NODE-HEALING の各ノードからの信号を単一のフリート全体のビューに統合します。これはまた、グローバル E-STOP がすべてのノードに一度に到達するために通る経路でもあります。
...
実際の、最小限の Rust スケルトンです——今日の時点でコンパイルと実行が
できます。
